...
class SmsGatewayZenviva(models.Model):
    ...
    _name = "sms.gateway.zenvivawa"
    _description = "Zenviva WA Gateway"
    ...
    def send_message(self, sms_gateway_id, from_number, to_number, sms_content, my_model_name='', my_record_id=0, media=None, queued_sms_message=None, media_filename=False):
        ...
        params = {
            'userkey': sms_account.zenvivawa_userkey,
            'passkey': sms_account.zenvivawa_passkey,
            'to': format_to,
            'message': sms_content,
        }

        response = requests.post(
            sms_account.zenvivawa_api_url,
            data=params
        )
...
class SmsAccountZenviva(models.Model):
    ...
    zenvivawa_api_url = fields.Char(string='API URL', default="https://console.zenziva.net/wareguler/api/sendWA/")
    zenvivawa_userkey = fields.Char(string='User Key')
    zenvivawa_passkey = fields.Char(string='Pass Key')
    zenvivawa_last_check_date = fields.Datetime(string="Last Check Date")