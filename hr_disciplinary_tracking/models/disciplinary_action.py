...
class DisciplinaryAction(models.Model):
    ...
    date = fields.Datetime(string='Date', required=True, readonly=True, index=True, states={'draft': [('readonly', False)]}, copy=False, default=fields.Datetime.now)
    ...
    def onchange_action(self):
        if self.action.name == 'Peringatan Tertulis' or self.action.name == 'Mutasi Jabatan':
            ...
            self.warning = 2
        elif self.action.name == 'Pemutusan Hubungan Kerja':
            self.warning = 3
